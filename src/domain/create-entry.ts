...
export class CreateEntryService {
  ...
  public async newEntry(rulesetRepo: RulesetRepository, bcrForkRepo: Repository, bcr: Repository, tag: string): Promise<string> {
  ...
  private async createEntryFiles(rulesetRepo: RulesetRepository, bcrRepo: Repository, tag: string): Promise<void> {
  ...
  private async commitEntry(rulesetRepo: Repository, bcrRepo: Repository, tag: string): Promise<string> {
    const repoAndVersion = `${rulesetRepo.canonicalName}@${tag}`;
    const branchName = repoAndVersion;

    await simpleGit(bcrRepo.checkoutPath)
      .addConfig("user.name", "Publish to BCR")
      .addConfig("user.email", "noreply@aspect.dev")
      .checkoutLocalBranch(branchName)
      .add("./*")
      .commit(`Publish ${repoAndVersion}`);

    return branchName;
  }

  private async pushEntry(bcrForkRepo: Repository, bcr: Repository, branch: string): Promise<void> {
    const authenticatedRemoteUrl = await this.githubClient.getAuthenticatedRemoteUrl(bcrForkRepo);

    await simpleGit(bcr.checkoutPath)
    .addRemote(
      "authed-fork",
      authenticatedRemoteUrl
    ).push("authed-fork", branch);
  }
}
...
function updateMetadataFile(sourcePath: string, destPath: string, version: string) {
  ...
  const metadata = JSON.parse(fs.readFileSync(sourcePath, {
    encoding: "utf-8",
  }));
...
function stampModuleFile(sourcePath: string, destPath: string, version: string) {
