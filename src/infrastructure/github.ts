...
export class GitHubClient {
  ...
  private async getRepoAuthorizedOctokit(repository: Repository): Promise<Octokit> {
  ...
  public async getForkedRepositoriesByOwner(owner: string): Promise<Repository[]> {
    ...
    return response.data
      ...
      .map(repo => 
        new Repository(repo.name, repo.owner.login, repo.html_url)
      );
  }

  public async getSourceRepository(repository: Repository): Promise<Repository|null> {
    ...
    if (repo.source) {
      return new Repository(repo.source.name, repo.source.owner.login, repo.html_url);
    }
    return null;
  }
  ...
  public async requestReview(repository: Repository, pullNumber: number, reviewers: string[]): Promise<void> {
    ...
    private async getRepositoryInstallation(repository: Repository): Promise<any> {
  ...
  public async getInstallationToken(repository: Repository): Promise<string> {
    if (!this._installationTokenCache[repository.canonicalName]) {
      const installationId = (await this.getRepositoryInstallation(repository)).id;
  ...
  public async getAuthenticatedRemoteUrl(repository: Repository): Promise<string> {
    const token = await this.getInstallationToken(repository);
