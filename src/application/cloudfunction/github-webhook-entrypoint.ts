...
const createEntryService = new CreateEntryService(githubClient);
const publishEntryService = new PublishEntryService(
  githubClient
);
const releaseEventHandler = new ReleaseEventHandler(githubClient, secretsClient, findRegistryForkService, createEntryService, publishEntryService);

export const handleGithubWebhookEvent: HttpFunction = async (request, response) => {
  const secretsClient = new SecretsClient();
  ...
  webhooks.on("release.published", event => releaseEventHandler.handle(event));
