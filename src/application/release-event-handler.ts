...
export class ReleaseEventHandler {
  ...
  public readonly handle: HandlerFunction<"release.published", unknown> = async event => {
    ...
    const candidateBcrForks = await this.findRegistryForkService.findCandidateForks(
      rulesetRepo,
      releaser
    );
    ...
    for (let bcrFork of candidateBcrForks) {
      try {
        ...
        const bcr = Repository.fromCanonicalName(process.env.BAZEL_CENTRAL_REGISTRY, true);
        ...
        await this.publishEntryService.createPullRequest(
          rulesetRepo,
          tag,
          bcrFork,
          bcr,
...
async function rulesetRepositoryFromPayload(payload: ReleasePublishedEvent): Promise<RulesetRepository> {
